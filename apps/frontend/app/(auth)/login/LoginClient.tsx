...
export default function LoginClient(props: { next: string; registered: boolean }) {
  const { next, registered } = props;
  ...
  async function onSubmit(values: FormValues) {
    ...
    setLoading(true);
    try {
      await service.login(values);               // sets httpOnly cookie via /api route
      router.replace(next || "/dashboard");      // go to dashboard (or ?next=…)
    } catch (e: any) {
